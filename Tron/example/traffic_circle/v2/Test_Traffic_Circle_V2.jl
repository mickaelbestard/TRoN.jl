...
function test_optim(;Nmax=8, final_time=0.3, Nc=3, itermax=1, optimizer=:grad, momentum=0, plot=false)
    ...
    ### graph visualization
    Net = traffic_circle_v2_graph(1., 8) 
    if plot 
        g = Net.graph
    ...
    @time Test_Traffic_Circle_V2(Tf=final_time, OPTIMIZER=optimizer, MOMENTUM=momentum, itermax=itermax, Nc=Nc, Nmax=Nmax)
end
...
function diagnostics(;data_path = "./data",
    ...
    push!(A_t, [9.99 0], [10 5], [15 10], [10 15], [20 10], [10 15], [5 10], [5 10])
    push!(B_t, [10 5], [15 10], [10 15], [10.01 20], [15 10], [5 10], [0 10], [10 5])
    ...
    for k = 1:1#4
        ...
        for Δt in Opt.Δt_t
            if abs(time - timeout[n]) < Δt
                ...
                filename = "heatmap_traffic_circle_v2_$(algo)_$(n).png"
...
println("load Test_Traffic_Circle_V2")